...
router.get('/animes', async (req, res, next) => {
  try {
    ...
    res.render('animes/animes-list', { animes: allAnimes });
  } catch (error) {
...
router.get('/animes/:animeId/edit', isLoggedIn, async (req, res, next) => {
  try {
    const { animeId } = req.params;
    const anime = await Anime.findById(animeId);
...
router.post('/animes/:animeId/edit', isLoggedIn, async (req, res, next) => {
  try {
    const { animeId } = req.params;
    const { title, author, description, rating } = req.body;
...
router.post('/animes/:animeId/delete', isLoggedIn, async (req, res, next) => {
  try {
    const { animeId } = req.params;
